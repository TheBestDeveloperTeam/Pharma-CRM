// notifications.js — Auto-generated Module
export function init(ctx) {
    ...
    async function loadData(page = 1) {
        ...
        const endpoint = /api/v1/admin/notifications.replace('-dashboard', '/dashboard');
        const res = await api('GET', endpoint + ?page= + page + &limit=20);
        
        if (!res || !res.data.success) { 
            view.replaceChildren(h('div', { class: 'empty-state' }, [h('p', { text: 'No notifications found or endpoint pending.' })]));
            return; 
        }
